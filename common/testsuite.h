...
static void mavlink_test_packed_param_values(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_PACKED_PARAM_VALUES >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_packed_param_values_t packet_in = {
    ...
    mavlink_packed_param_values_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.parameter_group_id = packet_in.parameter_group_id;
        packet1.number = packet_in.number;
        packet1.total_size = packet_in.total_size;
        
        mav_array_memcpy(packet1.buffer, packet_in.buffer, sizeof(uint8_t)*251);
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_PACKED_PARAM_VALUES_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_PACKED_PARAM_VALUES_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_packed_param_values_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_packed_param_values_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_packed_param_values_pack(system_id, component_id, &msg , packet1.number , packet1.total_size , packet1.parameter_group_id , packet1.buffer );
    mavlink_msg_packed_param_values_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_packed_param_values_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.number , packet1.total_size , packet1.parameter_group_id , packet1.buffer );
    mavlink_msg_packed_param_values_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_packed_param_values_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_packed_param_values_send(MAVLINK_COMM_1 , packet1.number , packet1.total_size , packet1.parameter_group_id , packet1.buffer );
    mavlink_msg_packed_param_values_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_packed_param_values(system_id, component_id, last_msg);
