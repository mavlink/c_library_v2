...
typedef struct __mavlink_battery_status_v2_t {
 ...
 float state_of_charge; /*< [%] State of Charge (SoC). Remaining capacity relative to current fully-charged capacity as a percentage. Values: [0-100], NaN: field not provided. Note that the value may be calculated from design_capacity if current full_charge_capacity is not known (see BATTERY_INFO).*/
...
#define MAVLINK_MSG_ID_BATTERY_STATUS_V2_LEN 27
#define MAVLINK_MSG_ID_BATTERY_STATUS_V2_MIN_LEN 27
#define MAVLINK_MSG_ID_369_LEN 27
#define MAVLINK_MSG_ID_369_MIN_LEN 27

#define MAVLINK_MSG_ID_BATTERY_STATUS_V2_CRC 145
#define MAVLINK_MSG_ID_369_CRC 145
...
#define MAVLINK_MESSAGE_INFO_BATTERY_STATUS_V2 { \
    ...
    "BATTERY_STATUS_V2", \
    8, \
    {  { "id", NULL, MAVLINK_TYPE_UINT8_T, 0, 26, offsetof(mavlink_battery_status_v2_t, id) }, \
         { "temperature", NULL, MAVLINK_TYPE_INT16_T, 0, 24, offsetof(mavlink_battery_status_v2_t, temperature) }, \
         ...
         { "state_of_charge", NULL, MAVLINK_TYPE_FLOAT, 0, 16, offsetof(mavlink_battery_status_v2_t, state_of_charge) }, \
         { "status_flags", NULL, MAVLINK_TYPE_UINT32_T, 0, 20, offsetof(mavlink_battery_status_v2_t, status_flags) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_BATTERY_STATUS_V2 { \
 ...
 * @param state_of_charge [%] State of Charge (SoC). Remaining capacity relative to current fully-charged capacity as a percentage. Values: [0-100], NaN: field not provided. Note that the value may be calculated from design_capacity if current full_charge_capacity is not known (see BATTERY_INFO).
                               ...
                               uint8_t id, int16_t temperature, float voltage, float current, float capacity_consumed, float capacity_remaining, float state_of_charge, uint32_t status_flags)
{
    ...
    _mav_put_float(buf, 16, state_of_charge);
    _mav_put_uint32_t(buf, 20, status_flags);
...
static inline void mavlink_msg_battery_status_v2_send(mavlink_channel_t chan, uint8_t id, int16_t temperature, float voltage, float current, float capacity_consumed, float capacity_remaining, float state_of_charge, uint32_t status_flags)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_BATTERY_STATUS_V2_LEN];
    _mav_put_float(buf, 0, voltage);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_battery_status_v2_send(chan, battery_status_v2->id, battery_status_v2->temperature, battery_status_v2->voltage, battery_status_v2->current, battery_status_v2->capacity_consumed, battery_status_v2->capacity_remaining, battery_status_v2->state_of_charge, battery_status_v2->status_flags);
#else
 ...
 */
static inline void mavlink_msg_battery_status_v2_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t id, int16_t temperature, float voltage, float current, float capacity_consumed, float capacity_remaining, float state_of_charge, uint32_t status_flags)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field state_of_charge from battery_status_v2 message
 *
 * @return [%] State of Charge (SoC). Remaining capacity relative to current fully-charged capacity as a percentage. Values: [0-100], NaN: field not provided. Note that the value may be calculated from design_capacity if current full_charge_capacity is not known (see BATTERY_INFO).
 */
static inline float mavlink_msg_battery_status_v2_get_state_of_charge(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_battery_status_v2_decode(const mavlink_message_t* msg, mavlink_battery_status_v2_t* battery_status_v2)
{
    ...
    battery_status_v2->state_of_charge = mavlink_msg_battery_status_v2_get_state_of_charge(msg);
